...
phab <- read_excel('../data/raw/phab_all.xlsx') %>% 
  filter(!VariableResult %in% 'Not Recorded') %>% 
  filter(!is.na(VariableResult)) %>% 
  mutate(VariableResult = as.numeric(VariableResult)) %>% 
  dplyr::select(StationCode, VariableResult) %>% 
  group_by(StationCode) %>% 
  dplyr::summarise(ch_alt = mean(VariableResult, na.rm = T)) %>% 
  ungroup %>% 
  mutate(ch_alt = round(ch_alt, 0))
...
# subset strmcat variables for most important across all regions
strmcatind <- strmcat[, c('COMID', impvrs)]
...
dat <- obsdat %>% 
  ...
  dplyr::select(COMID, StationCode, SiteSet, PSA6c, observed, predicted) %>% 
  mutate(resid = observed - predicted) %>% 
  left_join(strmcatind, by = 'COMID')

toplo <- dat %>% 
  filter(PSA6c %in% 'SC') %>% 
  gather('var', 'val', CBNFWs:PctUrbLo2011WsRp100)


ggplot(toplo, aes(x = val, y = resid )) + 
  geom_point() + 
