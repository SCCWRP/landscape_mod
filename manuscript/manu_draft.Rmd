...
```{r calimap, fig.height = 4.5, fig.width = 7, fig.cap = 'Land use (left) and distribution of stream \\ac{csci} scores (right) in California. Grey lines are ecoregions in California, CV: Central Valley, Ch: Chaparral, DM: Deserts Modoc, NC: North Coast, SN: Sierra Nevada, SC: South Coast.'}
...
```{r ridges, fig.height = 6, fig.width = 9, fig.cap = 'test'}
##
# plot data
...
# plot data
set.seed(12345)
rnd <- predict(rf_full, what = function(x) sample(x, 100, replace = T)) %>%
  data.frame %>%
  mutate(id = 1:nrow(.)) %>%
  .[sample(1:nrow(.), sel),] %>%
  gather('var', 'val', -id) %>%
  group_by(id) %>%
  mutate(medv = median(val)) %>%
  ungroup %>%
  arrange(medv) %>% 
  mutate(
    id = factor(id, levels = unique(id), labels = rev(1:sel)),
    id = as.character(id)
    ) 
  
# stream class labels, have to do manually
exp <- data.frame(
  id = as.character(1:sel), 
  exp = c('lu', 'pu', 'pu', 'pu', 'pu', 'pu', 'pc', 'lc', 'lc', 'lc'), 
  stringsAsFactors = F
  ) %>% 
  mutate(
    exp = factor(exp, 
                 levels = c('lu', 'pu', 'pc', 'lc'),
                 labels = c('likely unconstrained', 'possibly unconstrained', 'possibly constrained', 'likely constrained'))
  )

# site scrs
scr <- matrix(runif(40, 0, 1.4), ncol = 4) %>%  
  data.frame %>% 
...
rnd <- rnd %>% 
  left_join(exp, by = 'id') %>%
  left_join(scr, by = 'id') %>% 
  mutate(id = factor(id, levels = rev(1:sel)))

##
# plots

# stream class colors
exp_col <- RColorBrewer::brewer.pal(9, 'Paired')[c(6, 2, 5, 1)] %>% 
  alpha(., 0.8)  
...
# base 
pbase<- ggplot(rnd, aes(x = val, y = id)) + 
  geom_density_ridges(bandwidth = 0.1, alpha = 0.3, colour = NA, fill = 'grey', scale = ht) +
...
p1 <- pbase +
  geom_density_ridges_gradient(aes(fill = ..x..), bandwidth = bnd, rel_min_height = 0.001, colour = 'black', scale = ht) +
...
p2 <- pbase + 
  geom_density_ridges_gradient(aes(fill = ..x..), bandwidth = bnd, rel_min_height = min_ht, colour = 'black', scale = ht) + 
  geom_vline(xintercept = 0.79, linetype = 'dashed') + 
  scale_fill_gradient2(low = '#d7191c', mid = '#abd9e9', high = '#2c7bb6', midpoint = 1) + 
  ggtitle('(b) Expected scores with\nthreshold and cutoff')

p3 <- pbase + 
  geom_density_ridges(aes(fill = exp), bandwidth = 0.1, rel_min_height = min_ht, colour = 'black', scale = ht) +
  geom_vline(xintercept = 0.79, linetype = 'dashed') + 
  scale_fill_manual(values = exp_col) +
  ggtitle('(c) Stream reach classifications')

# p4 <- pbase + 
#   geom_density_ridges(aes(fill = exp), bandwidth = 0.1, rel_min_height = min_ht, colour = 'black', scale = ht) +
#   geom_point(aes(x = scr)) +
#   geom_vline(xintercept = 0.79, linetype = 'dashed') + 
#   scale_fill_manual(values = exp_col) +
#   ggtitle('(c) Stream reach classifications')

grid.arrange(p1, p2, p3, left = 'Stream reach', bottom = 'Expected CSCI scores', ncol = 3)
```
