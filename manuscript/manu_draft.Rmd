...
```{r calimap, fig.height = 4.5, fig.width = 7, fig.cap = 'Land use (left) and distribution of stream CSCI scores (right) in California. Grey lines are ecoregions in California, CV: Central Valley, Ch: Chaparral, DM: Deserts Modoc, NC: North Coast, SN: Sierra Nevada, SC: South Coast.'}

lucol <- c('lightgreen', 'lightyellow', 'lightgrey')
...
```{r sgrshd, fig.height = 5, fig.width = 7, fig.cap = 'San Gabriel River watershed in southern California with predicted median CSCI scores at each stream reach and differences of observed CSCI scores with the predictions. The watershed is undeveloped in the north and heavily urbanized in the south.  Gray stream reaches do not have model predictions because of insufficient data.'}

# land use data fortified  
toplo <- sgrbs %>% 
  fortify %>% 
  mutate(id = factor(id, levels = c(2, 1, 3), labels = c('Developed', 'Open, mixed', 'Water'))) %>% 
  rename(`Land cover` = id)

# hydrolines fortified
spatdat <- spat %>% 
...
# land use colors
lucol <- c('lightgrey', 'lightgreen', 'lightblue')
...
# prd csci seq for color pal
seqprd <- range(spatfrt$full0.50, na.rm = T) 
seqprd <- seq(seqprd[1], seqprd[2], length = 5)

# csci seq for color pal
seqcsc <- range(spatfrt$csci, na.rm = T) 
seqcsc <- seq(seqcsc[1], seqcsc[2], length = 5)

# csci dif seq for color pal
seqdif <- range(spatfrt$csci_diff, na.rm = T) 
seqdif <- seq(seqdif[1], seqdif[2], length = 5)

# separate lu legend
luleg <- ggplot() + 
  geom_polygon(data = toplo, aes(x = long, y = lat, group = group, fill = `Land cover`)) +
...
# base plot
pbs <- ggplot() + 
  geom_polygon(data = toplo, aes(x = long, y = lat, group = group), fill = pal_lu(toplo$`Land cover`), alpha = 0.5) + 
  geom_polygon(data = shedfrt, aes(x = long, y = lat, group = group), fill = 'white', alpha = 0.8) +
  coord_map() +
  theme_bw(base_family = 'serif') + 
  ...
  guides(
    ...
    colour = guide_legend(ncol = 4, title.position = 'top')
    ) 

# hydrolines plot with CSCI pts
p1 <- pbs + 
  geom_path(data = spatfrt, aes(x = long.x, y = lat.x, group = id, colour = full0.50)) +  
  scale_colour_gradientn('Reach predicted CSCI', colours = pal(seqprd)) +
  geom_point(data = spatfrt, aes(x = long.y, y = lat.y, fill = csci_diff), pch = 21, size = 3, alpha = 0.9) +
  scale_fill_gradientn('Observed CSCI difference with prediction', colours = pal_difr(seqdif)) + 
  ggsn::scalebar(location = 'bottomright', y.min = 33.77, y.max = 33.8, x.min = -118, x.max = -117.7, 
    dist = 10, st.dist=.3, st.size = 3, height = 0.3, dd2km = T, model = 'WGS84')
p1leg <- g_legend(p1) 
p1 <- p1 + theme(legend.position = 'none')

grid.arrange(
  p1, 
  arrangeGrob(luleg, p1leg, pinset, ncol = 1, heights = c(0.5, 1, 2)),
...
scrs <- c(1.35, 1.15, 0.84, 0.7, 1.25, 0.95, 0.75, 0.55, 1.1, 0.85, 0.7, 0.35, 1, 0.75, 0.45, 0.15)
...
toplo3 <- rnd %>% 
  select(id, estcut, estcat, prfcat) %>% 
...
toplo4 <- rnd %>% 
  select(id, scrs, estcat, prfcat) %>% 
  unnest %>% 
  mutate(
    prfcat = paste0(prfcat, ' (', estcat, ')'), 
    prfcat = factor(prfcat, levels = prflev),
    estcat = factor(estcat, levels = catcds$cls), 
    hght = scrs
  ) %>% 
  rename(`Site performance` = prfcat)
...
p4 <- pbase +
  geom_density_ridges(data = toplo3, aes(fill = `Stream class`), stat = 'identity', colour = 'black', scale = ht) +
  geom_vline(xintercept = thrsh, linetype = 'dashed') +
  geom_point(data = toplo4, aes(x = scrs, group = id), size = 3) + 
  geom_point(data = toplo4, aes(x = scrs, group = id, colour = `Site performance`), size = 2.5) + 
...
p1 <- ggplot(plot_ex, aes(x = typelv)) + 
  ...
  geom_point(aes(y  = `CSCI score`, fill = `Stream\nclass`), shape = 21, size = 4, alpha = 0.8) +
  geom_hline(yintercept = 0.79, linetype = 'dashed') +
  scale_colour_manual(values = pal_exp(levels(plot_ex$`Stream\nclass`)), 
