...
plot_ex <- proc_all(exps_ex, scrs_ex, thrsh = 0.79, tails = 0.05) %>% 
  ...
  mutate(
    `Stream\nclass` = factor(`Stream\nclass`, levels = levels(`Stream\nclass`), labels = c('lu', 'pu', 'pc', 'lc')), 
    `Relative\nperformance` = factor(`Relative\nperformance`, levels = levels(`Relative\nperformance`),
labels = c('os (lu)', 'e (lu)', 'us (lu)', 'os (pu)', 'e (pu)', 'us (pu)', 'os (pc)', 'e (pc)', 'us (pc)', 'os (lc)', 'e (lc)', 'us (lc)'))
  )
...
mythm <- theme_minimal(base_family = 'serif', base_size = 14) +
  theme(
...
p1 <- ggplot(plot_ex, aes(x = typelv)) + 
  ...
  geom_point(aes(y = medv, colour = `Stream\nclass`), size = 2, alpha = 0.9, shape = 15) +
  geom_point(aes(y  = `CSCI score`, fill = `Relative\nperformance`), shape = 21, size = 6, alpha = 0.8) +
...
grid.arrange(
  p1leg, 
  arrangeGrob(p1, p2, ncol = 2, widths = c(1, 0.4)), 
  heights = c(0.2, 1)
)
```
