...
Each site type was ranked as high, medium, or low priority for each action (Table \@ref(tab:pritab)).  No priority assigned to an action for a stream type was indication that baseline monitoring and maintenance was sufficient for a site type.
...
# table stuff
cap.val <- 'Priority actions defined by a stakeholder group for stream types in the SGR watershed.  Actions are generalized as investigate, protect, or monitor as high (H), medium (M), or low (L) priority.  All stream types are assumed to have baseline monitoring and maintenance (e.g., ongoing practices) such that priority actions are additional recommendations.  See Table \\@ref(tab:typetab) for a description of types and the text for definitions of each action.'

# table
knitr::kable(pritab, booktabs = T, caption = cap.val)
```
