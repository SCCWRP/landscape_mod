...
```{r sgrshd, fig.height = 6, fig.width = 4, fig.cap = 'test'}
...
# base plot
pbs <- ggplot() + 
  geom_polygon(data = toplo, aes(x = long, y = lat, fill = `Land cover`, group = group)) + 
  geom_polygon(data = shedfrt, aes(x = long, y = lat, group = group), fill = 'white', alpha = 0.4) +
  coord_map() +
  scale_fill_manual(values = lucol) +
...
p1 <- pbs + 
  ...
  scale_colour_gradientn('Median predicted\nCSCI', colours = pal(seqprd)) +
  geom_point(data = spatfrt, aes(x = long.y, y = lat.y, fill = csci_diff), pch = 21) +
  scale_fill_gradientn('CSCI difference', colours = pal_difr(seqdif))
