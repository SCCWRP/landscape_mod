...
pbase<- ggplot(toplo1, aes(x = xout, y = id, height = hght)) + 
  geom_density_ridges(stat = 'identity', alpha = 0.5, colour = NA, fill = 'darkgrey', scale = ht) +
...
p1 <- pbase +
  geom_density_ridges_gradient(aes(fill = ..x..), stat = 'identity', rel_min_height = 0.001, colour = 'black', scale = ht) +
...
p2 <- pbase +
  geom_density_ridges_gradient(data = toplo2, aes(fill = ..x..), stat = 'identity', colour = 'black', scale = ht) +
  scale_fill_gradient2(low = '#d7191c', mid = '#abd9e9', high = '#2c7bb6', midpoint = 1) +
...
grid.arrange(
  arrangeGrob(p1leg, p3leg, p4leg, ncol = 3, widths = c(0.3, 0.75, 1)),
  arrangeGrob(p1, p2, p3, p4, ncol = 4),
  left = 'Stream reach', bottom = 'Expected CSCI scores', ncol = 1, heights = c(0.2, 1)
)
```
