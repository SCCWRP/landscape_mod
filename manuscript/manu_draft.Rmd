...
p1 <- ggplot(caldat, aes(x = Longitude, y = Latitude, colour = RegImpQ)) +
  ...
  coord_map() + 
  guides(colour = guide_legend("Imperviousness, quartiles by region", title.position = 'top')) +
  theme_void(base_family = 'serif') + 
  theme(legend.position = 'top')

p2 <- ggplot(calmed) +
  geom_path(aes(x = long, y = lat, group = id, colour = full0.50), size = 0.20) +
  geom_polygon(data = psafrt, aes(x = long, y = lat, group = group), fill = NA, size = 0.5, colour = scales::alpha('black', 0.9)) +
  coord_map() + 
  scale_colour_gradientn(colours = pal(range(toplo$full0.50, na.rm = T))) +
  guides(colour = guide_colourbar('Reach predicted median CSCI', barheight = 0.5, barwidth = 8, title.position = 'top')) +
theme_void(base_family = 'serif') + 
  theme(legend.position = 'top')

png('figs/calires.png', height = 7, width = 8.5, units = 'in', res = 300, family = 'serif')
