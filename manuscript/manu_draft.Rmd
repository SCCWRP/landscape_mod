...
```{r calimap, fig.height = 4.5, fig.width = 10, fig.cap = 'Regions (left), land use (middle), and distribution of stream \\ac{csci} scores (right) in California.  Grey lines are county boundadries. Regions are CV: Central Valley, C: Chaparral, DM: Deserts Modoc, NC: North Coast, SN: Sierra Nevada, SC: South Coast.'}
...
# county base
calicnt <- map_data('county', region = 'california')
...
# psa plot
p1 <- ggplot() + 
  geom_polygon(data = toplo1, aes(x = long, y = lat, fill = Region, group = group)) + 
  geom_polygon(data = calicnt, aes(x = long, y = lat, group = group), fill = NA, colour = 'grey') + 
  ...
  ggsn::scalebar(location = 'bottomleft', y.min = 33, y.max = 35, x.min = -124, x.max = -120, 
           dist = 200, st.dist=.2, st.size = 4, height = 0.1, dd2km = T, model = 'WGS84')

p2 <- ggplot() + 
  geom_polygon(data = toplo2, aes(x = long, y = lat, fill = `Land use`, group = group)) + 
  geom_polygon(data = calicnt, aes(x = long, y = lat, group = group), fill = NA, colour = 'grey') +
  geom_polygon(data = calista, aes(x = long, y = lat, group = group), fill = NA, colour = scales::alpha('black', 0.5)) +
  coord_map() + 
  scale_fill_manual(values = lucol) +
...
p3 <- ggplot(csci_raw, aes(x = Longitude, y = Latitude, colour = CSCI)) +
...
grid.newpage()
v1 <- viewport(width = 1, height = 1, x = 0.22, y = 0.5) 
v2 <- viewport(width = 1, height = 1, x = 0.5, y = 0.5) 
v3 <- viewport(width = 1, height = 1, x = 0.78, y = 0.5) 
v4 <- viewport(width = 0.2, height = 0.2, x = 0.9, y = 0.65)
print(p1, vp = v1) 
print(p2, vp = v2)
print(p3, vp = v3)
print(pinset, vp = v4)
