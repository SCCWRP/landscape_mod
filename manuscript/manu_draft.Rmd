...
knitr::opts_chunk$set(fig.path = 'figs/', warning = F, message = F, echo = F, cache = T, dev = 'tiff', dev.args = list(family = 'sans', compression = 'lzw'), dpi = 500, warning = F, out.width = '100%', cache.path = 'manu_draft_cache/',
...
spelling::spell_check_files(here::here('manuscript', 'manu_draft.Rmd'))
```
...
# combined data to eval for perf, create factor altered/unaltered
prfdat <- obsdat %>% 
  ...
  mutate(
    obsalt = factor(ifelse(observed > 0.79, 'unaltered', 'altered')),
    prdalt = factor(ifelse(predicted > 0.79, 'unaltered', 'altered'))
  )

# get prediction accuracy
clsacc <- confusionMatrix(prfdat$obsalt, prfdat$prdalt) %>% 
  .$overall %>% 
  .[['Accuracy']] %>% 
  `*`(100) %>% 
  round
...
Model performance was comparable to similar studies that focused on developing predictions of biological condition from geospatial data.  @Hill17 developed a national model to predict stream site condition that correctly classified sites at about 75\% of locations, depending on region.  Importantly, regionally specific models were more accurate than a single national model.  For continuous predictions of biointegrity index scores, @Carlisle09 developed a model for a large area of the eastern United States.  Models for continuous data were able to correctly identify class membership from an a posteriori prediction at about 85\% of sites, which was similar in precision to models that were developed solely for categorical responses. For the landscape model herein, a comparison of the percentage of correctly classified sites that were above the 10th percentile of reference site scores (0.79) for observed data compared to predicted data showed that our model had comparable performance to other studies.  The landscape model had `r clsacc`\% predictive accuracy for classifying sites as altered (<0.79) or unaltered (>0.79) for the statewide results.  However, the goal of our model was distinct from previous studies, such that our intent was not to predict bioassessment scores at unsampled locations, but rather to describe variation in scores as a function of land use to identify constraints.  Interpretation of predictive accuracies between models should consider the differences in the goals for each model.
...
Geospatial data of model results mapped to stream reaches in California is provided at @Beck18d. The SCAPE model application website is available at [http://shiny.sccwrp.org/scape/](http://shiny.sccwrp.org/scape/), full source code accessible at @Beck18c. Additional content for the case study, figures, and tables are available in the supplement.
...
for(i in theta) {
  p1 <- p1 + geom_text_repel(data = psalab, 
                             aes_q(x = bquote(long +.(cos(i)*xo)),
                                   ...
                                   label = ~Region), point.padding = NA, parse = T, family = 'sans', size = 6.5, colour = 'black')
  
}
p1 <- p1 +
  geom_text_repel(data = psalab, aes(x = long, y = lat, label = Region), point.padding = NA, parse = T, family = 'sans', size = 6.5, colour = 'white') + 
  coord_map() + 
  ...
  labs(title = 'A', caption = 'Land use and regions')
...
p2 <- ggplot(csci_raw, aes(x = Longitude, y = Latitude, colour = CSCI)) +
  ...
  labs(title = 'B', caption = 'CSCI scores')
...
grid.newpage()
v1 <- viewport(width = 1, height = 1, x = 0.3, y = 0.51) 
v2 <- viewport(width = 1, height = 1, x = 0.7, y = 0.51) 
v3 <- viewport(width = 0.25, height = 0.25, x = 0.85, y = 0.65)

# tiff(here('manuscript/figs', 'calimap.tiff'), height = 5.5, width = 7, units = 'in', res = 500, family = 'sans', compression = 'lzw')
print(p1, vp = v1) 
print(p2, vp = v2)
print(pinset, vp = v3)
# dev.off()
...
# theme
pthm <- theme_minimal(base_family = 'sans') + 
  theme(
...
pbase1 <- ggplot(toplo1[toplo1$id %in% '3', ], aes(x = xout, y = id, height = hght)) + 
  geom_density_ridges(stat = 'identity', alpha = 0.5, colour = NA, fill = 'grey45', scale = ht) +
...
p1 <- pbase1 +
  geom_density_ridges(stat = 'identity', rel_min_height = 0.001, colour = 'black', fill = 'grey40', scale = ht) +
  ...
  labs(title = 'A', caption = 'Range of expected CSCI\nscores for stream segments')

p2 <- pbase1 +
  geom_density_ridges(data = toplo2[toplo2$id %in% '3', ], stat = 'identity', rel_min_height = 0.001, colour = 'black', fill = 'grey40', scale = ht) +
  scale_fill_gradient2(low = '#d7191c', mid = '#abd9e9', high = '#2c7bb6', midpoint = 1) +
  labs(title = 'B', caption = 'Expected CSCI scores\nwithin certainty range')
...
p3 <- pbase2 +
  ...
  guides(
    fill = guide_legend(title.position = 'top', nrow = 2, override.aes = list(colour = NA))
  ) +
  labs(title = 'C')
...
p3leg <- g_legend(p3)
p3 <- p3 + pthm %+replace% theme(legend.position = 'none')

p4leg <- g_legend(p4)
p4 <- p4 + pthm %+replace% theme(legend.position = 'none')

grid.arrange(
  arrangeGrob(textGrob(''), p3leg, p4leg, ncol = 3, widths = c(0.1, 1, 1)),
  arrangeGrob(
    arrangeGrob(
      textGrob('', rot = 90), p1, p2, ncol = 3, widths = c(0.1, 1, 1)
    ),
    arrangeGrob(
...
toplo <- caliclsplo %>% 
  mutate(
    strcls = factor(strcls, levels = levels(strcls), labels = c('Likely unconstrained', 'Possibly unconstrained', 'Possibly constrained', 'Likely constrained'))
  ) %>% 
  na.omit
...
p1 <- ggplot(toplo) +
  ...
  coord_map() +
  scale_colour_manual(values = pal_exp2(levels(toplo$strcls))) +
...
tiff('manuscript/figs/calires.tiff', height = 5.5, width = 4, units = 'in', res = 400, family = 'sans', compression = 'lzw')
p1
dev.off()
```
...
senscls <- sensres %>% 
  ...
  mutate(
    ...
    strcls = factor(strcls, 
                    levels = c('possibly unconstrained', 'likely unconstrained', 'likely constrained', 'possibly constrained'),
                    labels = c('Possibly unconstrained', 'Likely unconstrained', 'Likely constrained', 'Possibly constrained')
    )
  ) %>% 
...
# fill colors
fills <- pal_exp2(levels(senscls$strcls))
fills[c(1, 4)] <- NA
...
pres <- ggplot(senscls, aes(x = tails, y = perc, fill = strcls, group = strcls)) + 
  ...
  scale_x_discrete('Width of CSCI prediction range (wide to narrow)', expand = c(0, 0), breaks = c('0.05', '0.15', '0.25', '0.35', '0.45')) +
...
totab <- tibble(
  ...
  Definition = c('Lower bound of prediction range is above threshold', 'Lower bound of prediction range is below threshold, but median prediction is above', 'Upper bound of prediction range is above threshold, but median prediction is below', 'Upper bound of prediction range is below threshold'), 
...
```{r perftab}
# prfdat created in discussion chunk
