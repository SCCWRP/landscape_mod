---
title: "Achievable management goals using expectations of stream biological integrity relative to landscape constraints"
output: 
  pdf_document:
    keep_tex: true
    includes: 
...
Management decisions to restore or maintain aquatic resources are greatly informed by assessment tools that use biological indicators as surrogates of environmental condition.  A general goal is that a resource should support biota that exhibit the full range of elements and processes that are expected relative to a region’s natural environment.  This goal is impractical in many environments where large scale drivers on the landscape impose constraints on the expected upper limit of biological integrity.  We demonstrate application of a statewide landscape model to classify and prioritize stream monitoring sites using estimated constraints on biological integrity.  The model provides a context for what is likely to be achieved at a given site independent of an actual bioassessment score.  With this approach, sites can be ranked as over- or under-performing relative to an expectation that is appropriate for the landscape. Using a visualization tool based on this model, a local stakeholder group from the San Gabriel River watershed prioritized areas for different management actions.  The application provided context to identify a few under-performing regions (in both developed and undeveloped areas) from a dauntingly large number of low-scoring sites.  The application was also used to identify regions where protective measures or additional monitoring were needed.  Landscape models and interactive tools that effectively deliver an assessment context can help set achievable management goals.
